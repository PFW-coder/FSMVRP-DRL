...
class FSMVRPModel(nn.Module):
    ...
    def __init__(self, env_params, **model_params):
        ...
        self.decoder = FSMVRP_Decoder(self.env_params, **model_params)
    ...
    def forward(self, state):
        ...
        if state.selected_count == 0:  # First Move, depot
        ...
        elif state.selected_count == 1 and (self.training or self.model_params['eval_type'] == 'softmax'):
            ...
            probs = self.decoder(encoded_last_node, self.encoded_nodes, state.load, state.agent_capacity,
                                 state.agent_fixed_cost, state.agent_variable_cost, ninf_mask=ninf_mask)
            ...
            probs = self.decoder(encoded_last_node, self.encoded_nodes, state.load, state.agent_capacity,
                                 state.agent_fixed_cost, state.agent_variable_cost, ninf_mask=state.ninf_mask)
...
class FSMVRP_Decoder(nn.Module):
    def __init__(self, env_params, **model_params):
        super().__init__()
        self.model_params = model_params
        self.env_params = env_params
        ...
        self.context_linear = nn.Linear(2*embedding_dim+4, embedding_dim, bias=False)
        self.Wq_last = nn.Linear(2*embedding_dim+4, head_num * qkv_dim, bias=False)
        self.left_graph_embedding = nn.Linear(embedding_dim, embedding_dim, bias=False)
    ...
    def forward(self, encoded_last_node, encoded_nodes, load, agent_capacity, agent_fixed_cost, agent_variable_cost, ninf_mask):
        # encoded_last_node.shape: (batch, pomo, agent, embedding)
        # encoded_nodes: (batch, 1, problem, embedding)
        # load.shape: (batch, pomo, agent)
        # ninf_mask.shape: (batch, pomo, agent, problem)

        head_num = self.model_params['head_num']

        agent_fixed_cost_ = torch.where(load[:, :, :, None] == agent_capacity[:, None, :, None].repeat(1, self.pomo_size, 1, 1), agent_fixed_cost[:, None, :, None].repeat(1, self.pomo_size, 1, 1), 0)

        graph_embedding = encoded_nodes.unsqueeze(1).repeat(1, self.pomo_size, ninf_mask.size(2), 1, 1)

        graph_embedding = torch.where(ninf_mask.unsqueeze(-1) == 0, graph_embedding, 0).mean(-2)
        # shape: (batch, pomo, agent, embedding)

        graph_embedding = torch.where(load[:, :, :, None] == agent_capacity[:, None, :, None].repeat(1, self.pomo_size, 1, 1), graph_embedding, 0)

        #  Multi-Head Attention
        #######################################################
        input_cat = torch.cat((encoded_last_node, self.left_graph_embedding(graph_embedding), load[:, :, :, None], agent_capacity[:, None, :, None].repeat(1, self.pomo_size, 1, 1), agent_fixed_cost_, agent_variable_cost[:, None, :, None].repeat(1, self.pomo_size, 1, 1)), dim=3)
        # shape = (batch, pomo, agent, EMBEDDING_DIM+4)

        # graph_embedding mean()
        # 选择需要付出代价

        # 剩余的路线信息，将剩余的node embedding 的mean 进行输入
