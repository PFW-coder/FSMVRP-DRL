...
from FSMVRPProblemDef import get_random_problems, augment_xy_data_by_8_fold
...
class Reset_State:
    ...
    # shape: (batch, agent)
    agent_fixed_cost: torch.Tensor = None
    # shape: (batch, agent)
    agent_variable_cost: torch.Tensor = None
    # shape: (batch, agent)
...
class FSMVRPEnv:
    ...
    def load_problems(self, batch_size, aug_factor=1):
        ...
        if (not self.FLAG__use_saved_problems) and (not self.FLAG__use_random_seed):
            depot_xy, node_xy, node_demand, agent_capacity, agent_fixed_cost, agent_variable_cost = get_random_problems(batch_size, self.min_problem_size, self.max_problem_size, self.min_agent_num, self.max_agent_num)
        elif self.FLAG__use_random_seed:
            depot_xy, node_xy, node_demand, agent_capacity, agent_fixed_cost, agent_variable_cost = get_random_problems(batch_size,
            ...
            agent_fixed_cost = self.saved_agent_fixed_cost[self.saved_index:self.saved_index+batch_size]
            agent_variable_cost = self.saved_agent_variable_cost[self.saved_index:self.saved_index + batch_size]
        ...
        self.AGENT_IDX = torch.arange(self.agent_num)[None, None, :].expand(self.batch_size, self.pomo_size, self.agent_num)
    ...
    def reset(self):
        ...
        # shape: (batch, pomo, agent)
        self.used_cost = torch.zeros(size=(self.batch_size, self.pomo_size, self.agent_num))
        # shape: (batch, pomo, agent)
        self.agent_fixed_cost = self.reset_state.agent_fixed_cost
        # shape: (batch, agent)
        self.agent_variable_cost = self.reset_state.agent_variable_cost
        # shape: (batch, agent)
    ...
    def pre_step(self):
        ...
        self.step_state.used_cost = self.used_cost
        self.step_state.agent_capacity = self.agent_capacity
        self.step_state.agent_fixed_cost = self.agent_fixed_cost
        self.step_state.agent_variable_cost = self.agent_variable_cost
    ...
    def step(self, selected):
        ...
        self.used_cost += ((current_node_xy-last_node_xy)**2).sum(4).sqrt().sum(3) * (self.agent_variable_cost[:, None, :].repeat(1, self.pomo_size, 1))
        # shape: (batch, pomo, agent)

        fixed_cost_index = torch.where(last_node == self.current_node, 0, 1)
        fixed_cost_index = torch.where(last_node == 0, fixed_cost_index, 0)
        self.used_cost += fixed_cost_index * self.agent_fixed_cost[:, None, :].repeat(1, self.pomo_size, 1)
        ...
        # assert (self.visited_ninf_flag < 2).all()
        ...
        if done:
            reward = -self.get_total_cost()  # note the minus sign!
        else:
            reward = None
    ...
    def get_total_cost(self):
        ...
        current_node_xy = all_xy.gather(dim=3, index=self.current_node[:, :, :, None, None].repeat(1, 1, 1, 1, 2))
        # shape: (batch, pomo, agent, 1, 2)
        depot_xy = self.depot_node_xy[:, None, None, 0, :].unsqueeze(-2).repeat(1, self.pomo_size, self.agent_num, 1, 1)
        # shape: (batch, pomo, agent, 1, 2)
        left_cost = ((current_node_xy - depot_xy)**2).sum(4).sqrt().sum(3) * (self.agent_variable_cost[:, None, :].repeat(1, self.pomo_size, 1))
        # shape: (batch, pomo, agent)
        total_cost = torch.sum(left_cost + self.used_cost, dim=-1)
